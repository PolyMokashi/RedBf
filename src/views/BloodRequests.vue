...
export default {
  ...
  methods: {
   ...
   await axios
    .get("https://redb-backend.onrender.com/get/blood/request").then((result)=>{
...
console.log(this.list);
   ...
   async moveToAccepted(item) {
      ...
      item.rejected = true;
      await axios
        .post("https://redb-backend.onrender.com/post/status", {
          _id:item._id,
          name:item.name,
          stat:"accepted"
        })
...
console.log(item._id);
   await axios
    .delete(`https://redb-backend.onrender.com/delete/request/${item._id}`).then((data)=>{
console.log(data)
    }).catch((err)=>{
    console.log(err)
    })
